<template>
    <div class="z-30 fixed bottom-4 right-4">
      <!-- Floating Button -->
      <button
        @click="toggleShareOptions"
        class="bg-blue-500 opacity-70 hover:bg-blue-600 hover:opacity-90 text-white rounded-full p-4 shadow-lg focus:outline-none"
      >
        <!-- Heroicons Share Icon -->
        <XMarkIcon v-if="toggleActive" class="h-6 w-6" />
        <ShareIcon v-else class="h-6 w-6" />
      </button>
  
      <!-- Share Options -->
      <div
        v-if="showOptions"
        class="absolute bottom-16 right-0 bg-white rounded-full shadow-lg p-2 space-y-2"
      >
        <!-- Facebook -->
        <a
        ...
        <button
          ...
          :disabled="copied"
        >
          <CheckIcon v-if="copied" class="h-10 w-10 mx-auto rounded-full p-2 text-white bg-green-400"/>
          <LinkIcon v-else class="h-10 w-10 mx-auto rounded-full p-2 bg-gray-400"/>
        </button>
      </div>
    </div>
  </template>
...
<script setup>
    ...
    import { ShareIcon, LinkIcon, CheckIcon, XMarkIcon } from '@heroicons/vue/24/outline'
